...
public class CartDomainTests
{
        ...
        int itemCount)
    {
        ...
        Assert.Equal(cart.Subtotal, new Money(expectedSubtotal, Currency: expectedCurrency));
        Assert.Equal(cart.Items.Count, expectedItemsCount);
        Assert.Equal(cart.Items[itemCount].Quantity, Quantity.From(expectedProductQuantity));
    }
}